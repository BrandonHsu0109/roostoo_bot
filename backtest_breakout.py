...
def backtest_breakout(
    ...
    lookback: int = 60,                 # breakout window in minutes
    buffer_pct: float = 0.001,          # breakout buffer (0.1%)
    vol_window: int = 60,               # sizing volatility window
    top_n: int = 5,
    ...
    for i in range(len(mid.index)):
        ...
        # Breakout condition: mid > rolling_high*(1+buffer)
        window = mid.iloc[i - lookback: i]  # previous lookback minutes
        rolling_high = window.max()
        signal = (mid.iloc[i] > rolling_high * (1.0 + buffer_pct))

        eligible = base_ok & signal

        # Vol for sizing
        vol = rets.iloc[i - vol_window + 1: i + 1].std(ddof=1)

        # Score: signal / vol (only for eligible)
        score = (1.0 / (vol + eps)).where(eligible, other=np.nan).dropna()

        # Target all cash by default (sell when no signals)
        target = {p: 0.0 for p in pairs}

        if len(score) > 0:
            top = score.sort_values(ascending=False).head(int(top_n))
            selected = top.index.to_list()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(r"Usage: python backtest_breakout.py logs\ticker_history.csv")
        raise SystemExit(1)
    backtest_breakout(sys.argv[1])
...
def backtest_meanrevert(
    csv_path: str,
    initial_usd: float = 50000.0,
    z_window: int = 60,
    z_entry: float = -1.5,          # buy when oversold
    z_exit: float = -0.3,           # exit when mean-reverted
    vol_window: int = 60,
    top_n: int = 5,
    ...
    need = max(z_window+1, vol_window+1)
    ...
    for i in range(len(mid.index)):
        ...
        # z-score
        window = mid.iloc[i-z_window:i]
        ma = window.mean()
        sd = window.std(ddof=1)
        z = (mid.iloc[i] - ma) / (sd + eps)

        # target all cash by default (exits happen naturally)
        target = {p: 0.0 for p in pairs}

        # mean-reversion entry: buy oversold
        eligible = base_ok & (z <= float(z_entry))
        # exit rule is handled by target=0 when not eligible; to reduce churn, we can keep holding
        # until z >= z_exit by allowing "hold" on positions:
        for p in pairs:
            if pos[p] > 0:
                # if still below exit threshold, keep holding by setting target to current
                if pd.notna(z[p]) and float(z[p]) < float(z_exit):
                    target[p] = float(pos[p])

        # choose oversold coins to buy (score by inverse vol)
        vol = rets.iloc[i-vol_window+1:i+1].std(ddof=1)
        score = (1.0 / (vol + eps)).where(eligible, other=np.nan).dropna()

        if len(score) > 0:
            top = score.sort_values(ascending=False).head(int(top_n))
            investable = float(pv) * (1.0 - float(cash_buffer))

            w = top.to_numpy(dtype=float)
            w = w / float(np.sum(w))
            w = np.minimum(w, float(max_weight_per_coin))
            w = w / float(np.sum(w))

            for p, wi in zip(top.index.to_list(), w):
                m = mid.iloc[i][p]
                if pd.notna(m) and float(m) > 0:
                    target[p] = max(target[p], (investable * float(wi)) / float(m))

        # execute at most K orders (BUY threshold applies, SELL always allowed)
        cands = []
        for p in pairs:
            ...
            if delta > 0:
                side="BUY"; px=ask.iloc[i][p]; qty=float(delta)
            else:
                side="SELL"; px=bid.iloc[i][p]; qty=float(-delta)
...
if __name__=="__main__":
    if len(sys.argv)<2:
        print(r"Usage: python backtest_meanrevert.py logs\ticker_history.csv")
        raise SystemExit(1)
    backtest_meanrevert(sys.argv[1])